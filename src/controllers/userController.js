...
try{
 let data=req.body;
 let liveWeather=await weatherModel.findById({_id:"6565d3a1ccb55e9d9347d0dd"})//.select({_id:0,cityId:1,time:1,temperature:1,maxTemperature:1,minTemperature:1,humidity:1,windSpeed:1,condition:1})
...
return res.status(201).send({msg:"login successfull",token,liveWeather})
}
catch(error){
console.log(error)
res.status(500).send(error)
